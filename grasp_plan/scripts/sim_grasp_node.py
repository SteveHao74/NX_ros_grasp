...
def callback(data):   
    global publisher,grasp_planner,counter,plan_flag,listener,grasp_msg
    counter = counter+1
    ##########subcribe_stage###########
    bridge = CvBridge()
    ...
    if plan_flag :  
        ...
        if grasp_describer is None:
            ...
            # temp_RPY=rotationMatrixToEulerAngles(g_base[:3,:3])
            print("对比",RPY,temp_RPY)
            tran = g_base[:3,3]
        ...
        #########publish_stage###########        
            ...
            grasp_msg.translation_speed=0.05#0.1 # 米
            grasp_msg.orientation_speed=5#15 # 度
            publisher.publish(grasp_msg)
            rospy.loginfo("Publsh grasp message pos[%f, %f, %f],ori[%f, %f, %f,%f]", 
                        grasp_msg.position_x, grasp_msg.position_y, grasp_msg.position_y,grasp_msg.orientation_x,grasp_msg.orientation_y,grasp_msg.orientation_z,grasp_msg.orientation_w)
...
def displayWebcam():
    ...
    global publisher,grasp_planner,listener,grasp_msg
