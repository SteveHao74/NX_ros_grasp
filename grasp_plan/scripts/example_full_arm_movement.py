...
class ExampleFullArmMovement:
    ...
    def example_send_cartesian_pose(self,grasp_msg):
        self.last_action_notif_type = None
        ...
        req.input.target_pose.x = grasp_msg.position_x
        req.input.target_pose.y = grasp_msg.position_y 
        req.input.target_pose.z = grasp_msg.position_z
        req.input.target_pose.theta_x = grasp_msg.angle_x
        req.input.target_pose.theta_y = grasp_msg.angle_y
        req.input.target_pose.theta_z = grasp_msg.angle_z

        pose_speed = CartesianSpeed()
        pose_speed.translation = grasp_msg.translation_speed
        pose_speed.orientation = grasp_msg.orientation_speed
        rospy.loginfo("speed[%f, %f, ", grasp_msg.translation_speed,grasp_msg.orientation_speed)
        ...
        # Call the service
        # print(feedback.base.commanded_tool_pose_theta_x,feedback.base.commanded_tool_pose_theta_y,feedback.base.commanded_tool_pose_theta_z)
    ...
    def example_send_init_pose(self,grasp_msg,pos_list):
        self.last_action_notif_type = None
        # Get the actual cartesian pose to increment it
        # You can create a subscriber to listen to the base_feedback
        # Here we only need the latest message in the topic though
        ...
        req = PlayCartesianTrajectoryRequest()
        req.input.target_pose.x = pos_list[0]
        req.input.target_pose.y = pos_list[1]
        req.input.target_pose.z = pos_list[2]
        req.input.target_pose.theta_x = pos_list[3]
        req.input.target_pose.theta_y = pos_list[4]
        req.input.target_pose.theta_z = pos_list[5]
    ...
    def example_send_joint_angles(self,tar_angle):
        self.last_action_notif_type = None
    ...
    def example_send_gripper_command(self, value):
        ...
        req.input.mode =GripperMode.GRIPPER_POSITION#GripperMode.GRIPPER_FORCE# GripperMode.GRIPPER_POSITION
