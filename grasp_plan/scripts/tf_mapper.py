...
def np2ur(pose):
    rotv = Rotation.from_matrix(pose[:3, :3]).as_rotvec()
...
class TFMapper_EyeInHand(object):
    ...
    # z = Rotation.from_euler('z', 135, degrees=True).as_matrix()
    T_b_w = np.eye(4)
    # T_b_w[:3, :3] = x.dot(z)
...
###########################################
    # T_b_w = np.eye(4)
    # T_b_w[:3, :3] = Rotation.from_euler('zyz', [45,45, 90], degrees=True).as_matrix()
    # T_b_w[:3, 3] = [0, 0.225, 0.4]
    # T_w_b = np.linalg.inv(T_b_w)
    ...
    def base_to_world(self,T_g_b):
        """ 基座坐标系变换到世界坐标系 """
        T_g_w = self.T_b_w.dot(T_g_b)
        return T_g_w
    ...
    def grasp_to_matrix_pro(self, p0, p1, center, T_t_w):
        ...
        m0 = self.graspaxis_in_world(p0, p1,center, T_t_w)#得到了target相对于world的齐次变换阵
        # xy轴都相反z轴不变
        m1 = m0.copy()
        m1[:3, :2] = -m1[:3, :2]#把X，Y轴都取反
        r0 = np.linalg.inv(m0[:3, :3]).dot(T_t_w[:3, :3])
        c0 = np.sum(np.abs(Rotation.from_matrix(r0).as_euler('xyz')))
        r1 = np.linalg.inv(m1[:3, :3]).dot(T_t_w[:3, :3])
        c1 = np.sum(np.abs(Rotation.from_matrix(r1).as_euler('xyz')))
    ...
    @staticmethod
    def graspaxis_in_world(p0, p1,center,T_c_w):
        """ 计算在抓取路径, 即与z轴最近的方向,这个是在物体坐标系下计算 """
        # 这里的y轴正反方向不确定,需要执行的时候再确定
        x = p0 - p1
        x = x / np.linalg.norm(x)#
...
class TFMapper_EyeToHand(TFMapper_EyeInHand):
    ...
    def grasp2d_to_matrix(self, g, T_g_b, grasp_depth):#这里出问题！！！！！
        ...
        # p0[2] = (p0[2]+p1[2]) /2
        # # p1[2] = (p0[2]+p1[2]) /2
        p0[2] = center[2]#self.dis_cam_to_table-grasp_depth。。。。#此处就是强行拉平
        p1[2] = center[2]#self.dis_cam_to_table-grasp_depth。。。。#此处就是强行拉平
        print(center)
        print('----------------p0', p0)
        print('----------------p1', p1)
